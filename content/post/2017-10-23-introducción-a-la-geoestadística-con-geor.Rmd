---
title: Introducción a la Geoestadística con `geoR`
author: Rubén Fernández Casal
date: '2017-10-23'
slug: introducción-a-la-geoestadística-con-geor
categories: ["Apuntes", "R"]
tags: ["Geoestadística", "regresion", "Paquetes"]
summary: "El paquete `geoR` proporciona herramientas para el
  análisis de datos geoestadísticos en `R`
  (otra alternativa puede ser emplear el paquete `gstat`, por ejemplo...).
  En este post se ilustran algunas de las capacidades de este paquete."
output:
  blogdown::html_page:
    toc: true  
---
...
# las opciones globales de renderizado...
# knitr::opts_chunk$set(echo = FALSE, fig.width = 4, fig.height = 4, fig.align = 'center')
knitr::opts_chunk$set(comment = "   #", cache = TRUE, fig.width = 7, fig.align = 'center')
```
...
El paquete `geoR` proporciona herramientas para el análisis de datos
...
Para verificar si un modelo de variograma describe adecuadamente la
dependencia espacial de los datos (p.e. comparar modelos), se emplea
...
observados menos predicciones) y los segundos con residuos
...
if(!require(npsp)) 
  stop('Required pakage `npsp` not installed.') # install.packages('npsp')
...
Copyright 2009-2018 Rubén Fernández Casal

Se autoriza la realización y distribución de copias literales de este
manual, siempre y cuando las advertencias del copyright y de este
permiso se conserven en todas las copias.

Se autoriza la realización y distribución de copias modificadas de este
manual, en las mismas condiciones de las copias literales, siempre y
cuando la totalidad del trabajo resultante se distribuya bajo los
términos de una advertencia de permiso idéntica a esta.
